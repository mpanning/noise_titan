...
ppsd = PPSD(tr_tmp.stats, paz, db_bins=[-300, -75, 5],
            period_limits=[0.5, 500], ppsd_length=3600.0)
secday = 24.0 * 3600.0
...
for filename in filenames:
    ...
    while (iday > 365):
       year = year + 1
       iday = iday -365
    st[0].stats.starttime = UTCDateTime("%4d-%03dT00:00:00.0" % (year, iday))
...
for i in range(len(Pdb)):
    psd_pd = 1.0/freq[i]
    ax.plot(psd_pd, Pdb[i], color=psd_color, linestyle = '-', linewidth=2.0)
ax.plot(0., 0., color=psd_color, label=psd_label, linewidth=2.0)
...
# Add in instrument noise curves
for seis in seis_types:
    ax.plot(period[seis], psd[seis], color=color[seis], linestyle=style[seis],
            linewidth=size[seis], label=seis)
